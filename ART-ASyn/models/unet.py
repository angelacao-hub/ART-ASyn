...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb=None, encoder_out=None):
        for layer in self:
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            normalization_bn(channels, swish=1.0),
            nn.Identity(),
        ...
        self.out_layers = nn.Sequential(
            normalization_bn(self.out_channels, swish=0.0 if use_scale_shift_norm else 1.0),
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm = normalization_bn(channels, swish=0.0)
        # self.norm = normalization_in(channels, swish=0.0)
...
class UNetModel(nn.Module):
    ...
    def sample_from_q(self, mu, log_sigma_sq):
        epsilon = Variable(th.randn(mu.size()), requires_grad=False).type(th.cuda.FloatTensor)
        sigma = th.exp(log_sigma_sq / 2)
    ...
    def forward(self, x, conditioning_x=None):
        """
